...
| `test_shell.py` | `lib/common.sh` (`server_hostport` parsing incl. IPv6 literals; `wait_for_server` happy-path / timeout / recovery / skip against a stubbed `nc`; `recovery_budget` mode selection and the `NET_WAIT_MAX >= THROTTLE_INTERVAL` invariant), `bin/vpnc-slice` (`@server` expansion + unsafe-domain filtering, resolver sweep incl. the keep-list and marker isolation, the proxy-ownership invariant `_may_touch_proxy` / `_is_dnsroute` / failed-bind), `install-autostart.sh` (`dir_is_safe` + `verify_safe_ancestors` for the NOPASSWD-helper path, and `dir_ok_for_repo` + `verify_repo_ancestors` for the user's repo-path ancestor walk), and `bin/openconnect-auto-sso` via the `OC_CONNECT_TEST=1` seam (`recovery_budget` wiring through `OC_LAUNCHD`, `_shq` quoting, the Phase-2 `-s` / `_build_vs` assembly, the `$PROJ` ERE-metacharacter guard). macOS-only (skipped elsewhere: BSD `stat -f`, `/etc/resolver`). |
...
- `OC_VPNC_SLICE_TEST=1` / `OC_INSTALL_TEST=1` / `OC_CONNECT_TEST=1` — source-guards that stop each script before its main body, leaving only the functions (and, for the connect script, the pure helpers `_shq` / `apply_launch_budget` / `_build_vs` plus the `$PROJ` guard) defined, with no config load / auth / sudo.
